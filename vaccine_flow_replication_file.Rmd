...
moderna_shipping <-
  read_csv(
    "../data/COVID-19_Vaccine_Distribution_Allocations_by_Jurisdiction_-_Moderna.csv"
  ) %>%
...
pfizer_shipping <-
  read_csv(
    "../data/COVID-19_Vaccine_Distribution_Allocations_by_Jurisdiction_-_Pfizer.csv"
  ) %>%
  filter(Jurisdiction == "Total") %>%
  select(-1,-2) %>%
  mutate_all( ~ str_replace_all(., ",", "") %>% as.numeric)
...
schedule[[1]][["moderna"]] <- tibble(week = c(1, 2, 3, 4),
                                     people = pfizer_shipping[, c(10, 12, 14, 16)] %>% unlist(use.names = F))
